...
KindEditor.plugin('filemanager', function (K) {
    var self = this, name = 'filemanager',
		fileManagerJson = K.undef(self.fileManagerJson, self.basePath + 'upload/file_manager_json.ashx'),
    ...
    self.plugin.filemanagerDialog = function (options) {
        ...
        function createView(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                if (data.is_photo) {//如果是图片 如果是file|| data.is_dir
                    var _span = K('<span class="xl_span" data-url="' + K.formatUrl(result.current_url + data.filename, 'absolute') + '" style="position:absolute;display:none;width:102px;background:#0690d2;color:#FFF;text-align:center; cursor: pointer;line-height:20px;bottom:-3px;left:0;">Delete</span>');
                    div.append(_span);
                }
            }
            ...
            K(".xl_span").click(function () {//这里的.xl_span对应着刚才插入的删除按钮上的class
                var $this = K(this);
                if (!confirm("Are You Sure To Delete This Image Now?")) {//提示，如果点击取消则直接退出
                    return false;
                }
                K.ajax(K.addParam(fileManagerJson, 'action=delete&url=' + $this.attr("data-url") + '&' + new Date().getTime()), function (res) {
                    res == "1" ? $this.parent().remove() : alert(lang.deleteError);
                    if (K(".ke-plugin-filemanager-body").children().length < 1) { K(".ke-plugin-filemanager-body").html(lang.noImg) }
                });
            })
